...
def get_data_from_release(candidates_file, release_file, threshold=0.99):
    ...
    for chunk in tqdm(data, total=9729, desc="Data progress"):
        ...
        merge_df = pd.merge(candidates, chunk_df, on=["oid", "flare_start"], how="inner")
...
def asteroid_search(ra, dec, time_max):
    ...
    try:
        results = Skybot.cone_search(field, RADIUS_SEARCH_ASTEROID * u.deg, earthed_time, location="I41")
...
def search_asteroids(df: pd.DataFrame):
    ...
    for i, obs in tqdm(df.iterrows(), total=df.shape[0]):
        try:
            asteroid, asteroid_name = asteroid_search(obs["ra"], obs["dec"], obs["time_maximum"])
        except Exception as e:
            if len(e.args) > 0:
...
def get_sharpness(df):
    ...
    for i, obs in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        url_request = URL_CALTECH + str(obs["oid"]) + "&TIME=" + str(start_time_request) + "%20" + str(end_time_request)
